...
c 18.6.2018 => KLR from version xsidre-0.3.8.f: 
c                     - removed udmget dependency in ky model code
...
c-----------------------variables declarations--------------------------------
     ...
     $                       cmodel,edivision,min,max,nbins_final)
      ...
      real am_old,thetaO_old,normal,am,am2
      real min,max,Enorm,gmin,gmax,gmin_loc,gmax_loc
      real alpha,beta,rb,drad,pom,pom1,pom2,pom3,r_plus
      real rin,rms,rout,thetao,xlow,xhigh,zshift,zzshift,zzshift2
      integer ntable_old,nbins_old,i,ibin,ie,ie_loc,ierr
      integer ihigh,ilow,ir,ir0,irn,nr
...
c       to total flux equal to unity
       ...
       if(photar_int.gt.1e-30)then
        do i=1,ne
         photar(i)=SNGL(photar1(i)/photar_int)
        enddo
       endif
      else
       do i=1,ne
        photar(i)=SNGL(photar1(i)) 
       enddo
      endif
9999  continue
      if(ierr.ne.0)then
...
c     pointers to arrays from idre...
      ...
      real gmin,gmax,thetaOO,flower,fupper,gdelta,gdelta_final
      real ghigh,ghigh_final,glow,glow_final,rm,ttmp,ttmp1,utmp
      real utmp1,y1,y2,y3,y4
      integer ntable_old,i,iam0,ibin,ibin_final,ihorizon,iincl
      integer irow,ith0,j,nelements,nrow
...
c these are needed to work with the fits file...
      ...
      real nulle
      double precision tscal,tzero,pi,pi2
...
c       let's free the unit number
        ...
        ntable=-1
        errortxt=
     $   'idre: Cannot find the KY tables'
