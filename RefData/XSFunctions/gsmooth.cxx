// Convolution model with a gaussian kernel whose width can vary with energy
// Parameters:
//     0    Gaussian sigma at 6 keV
...
void gsmooth(const RealArray& energyArray, const RealArray& params,
	     int spectrumNumber, RealArray& flux, RealArray& fluxErr, 
	     const string& initString)
{
  size_t nE = energyArray.size();

  // construct the array of gaussian sigmas
  ...
  // gaussians with the center in each energy bin and the normalization based
  // on the flux in that bin

  RealArray inputFlux(flux);
  flux = 0.0;
  calcManyLines(energyArray, centerEnergy, width, inputFlux, (Real)1.0e-6, 0,
		true, flux);
}
