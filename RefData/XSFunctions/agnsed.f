...
c     param(6) Te of hard compton 
c     param(7) Te of warm compton
c     param(8) gamma hot 
c     param(9) gamma warm
c     param(10) Rhot
c     param(11) Rwarm 
c     param(12) log10 rout/rg 
c     param(13) Htmax is upper limit of the corona scale height (1
...
c------- set rcor------
      if (rcor.le.rpow)then
         ...
         rcor=rpow
        print *, "==========================================="
        print *, "Rwarm <= Rhot <reset as Rwarm=Rhot>"
        print *, "==========================================="
      endif
c--------------------
c------- set rcor------0109_rev
      if (rcor.gt.rsg)then
         rcor=rsg
        print *, "==========================================="
        print *, "Rwarm >  Rout <reset as Rwarm=Rout>"
        print *, "==========================================="
      endif
c--------------------
...
c     now add high temp compton emission
      ...
      write(comment,*)'------hot compton-----'
      CALL xwrite(comment, 20)
      write(comment,*)'gamma_hot=',gammah
      CALL xwrite(comment, 20)
      write(comment,*)'T(Rhot)nt=',
     & amytempreph(m,astar,mdot,rms,rpow,rep,lumipl,Ht,a)
      CALL xwrite(comment, 20)
      write(comment,*)'Ldis,hot/Ledd=',displ*4.0*pi*d*d/ledd,
     & 'Lhot/Ledd=',  lumipl*4.0*pi*d*d/ledd
      CALL xwrite(comment, 20)
      write(comment,*)'------warm compton-----'
      CALL xwrite(comment, 20)
      write(comment,*)'T(Rwarm)=',amytempreph(m,astar,mdot,rms,rcor,rep,
     &        lumipl,Ht,a)
      CALL xwrite(comment, 20)
      write(comment,*)'------Rout-----'
      CALL xwrite(comment, 20)
      write(comment,*)'rout=', rsg, 'tout=', tsg
      CALL xwrite(comment, 20)
      write(comment,*)'-----------'
      CALL xwrite(comment, 20)
