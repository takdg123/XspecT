...
void calc_xscat(const RealArray& energyArray, 
  ...
  size_t iE, Nrext, Nxpos;
  int ii, jj, kk, rl, rh, xl, xh;
  ...
  for (jj=0;jj<((signed int) Nxpos-1);jj++) {
  ...
  if ((xl<0)||(xh<0)||(rl<0)||(rh<0)) {
    errMsg << "\nxscat:\nDust position (" << xpos << 
      ") not in range 0->1 or extraction region (" << rext <<
      ") out of bounds.\n";
...
Real interpol_huntf(int n, Real *x, Real *y, Real z) {
  ...
  if (( inc && (z < x[0] || z > x[n-1])) ||
      (!inc && (z > x[0] || z < x[n-1]))) {
    std::ostringstream errMsg;
    errMsg << "interpol_huntf: Asking for " << z << ", min is " << x[0] 
	   << ", max is " << x[n-1] << ": Cannot extrapolate";
    xs_write(const_cast<char*>(errMsg.str().c_str()),10);
    if ( (inc && z < x[0]) || (!inc && z > x[0]) ) return x[0];
    if ( (inc && z > x[n-1]) || (!inc && z < x[n-1]) ) return x[n-1];
  }  
  jl = 0;
  ju = n;
  while (ju - jl > 1) {
