...
class FunctionUtility 
{
  ...
  public:
      ...
      static const string& neiVersion ();
      static void neiVersion (const string& value);
      ...
      static void tempsDEM (const std::vector<double>& TempArray);
      static void tempsDEM (const RealArray& TempArray);
      static std::vector<double>& DEM ();
      static void DEM (const std::vector<double>& DEMArray);
      static void DEM (const RealArray& DEMArray);
      ...
      static void xsWrite(const string output, const int chatterLevel);
...
inline const string& FunctionUtility::neiVersion ()
{
  return s_neiVersion;
}

inline void FunctionUtility::neiVersion (const string& value)
{
  s_neiVersion = value;
}
...
inline void FunctionUtility::tempsDEM (const std::vector<double>& TempArray)
{
  s_tempsDEM.resize(TempArray.size());
  for (size_t i=0; i<TempArray.size(); i++) s_tempsDEM[i] = TempArray[i];
  return;
}

inline void FunctionUtility::tempsDEM (const RealArray& TempArray)
{
  s_tempsDEM.resize(TempArray.size());
  for (size_t i=0; i<TempArray.size(); i++) s_tempsDEM[i] = TempArray[i];
  return;
...
inline void FunctionUtility::DEM (const std::vector<double>& DEMArray)
{
  s_DEM.resize(DEMArray.size());
  for (size_t i=0; i<DEMArray.size(); i++) s_DEM[i] = DEMArray[i];
  return;
}

inline void FunctionUtility::DEM (const RealArray& DEMArray)
{
  s_DEM.resize(DEMArray.size());
  for (size_t i=0; i<DEMArray.size(); i++) s_DEM[i] = DEMArray[i];
  return;
...
{
  if ( s_abundanceVectors.count(table) == 1 ) s_abundanceVectors.erase(table);
