...
subroutine thcompf(ear,ne,param,IFL,photar,photer) 
  IMPLICIT NONE
  integer ifl,ne,i,j
  double precision cov_frac

  real ear(0:ne),param(4),photar(ne),photer(ne),kte,tau,gam_tau,gamma,z_red
  double precision theta0,theta,alp,tauthin,tauthick,a,b,c,q,delta
  real thpar(4),tmparrt(ne)
  real psum,tsum
  ...
  if(gam_tau.gt.0.0)then
     ...
     WRITE(comment, '(2(a,1pg14.7))') &
          "THCOMP info: gamma parametrization, tau =", tau
     CALL xwrite(comment, 15)
  else
     tau=-gam_tau
     gamma=sqrt(1.0/(tau*(tau*a+b))+2.25)-0.5
     
     WRITE(comment, '(2(a,1pg14.7))') &
          "THCOMP info: tau parametrization, gamma =", gamma
     CALL xwrite(comment, 15)
  endif
  ...
  photar = real(tmparrt/tsum*psum*cov_frac+(1.0-cov_frac)*photar) ! normalize the output
