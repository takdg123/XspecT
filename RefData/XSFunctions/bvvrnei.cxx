//    Bvvrnei model
//	Broadened recombining plasma model. This model requires NEIVERS 3.0 and above.
//		   parameters (param):
...
void bvvrnei(const RealArray& energyArray, const RealArray& params,
	     ...
	     const string& initString)
{
  ...
  Real velocity(params[34]);
  Real Tinput(params[0]);

  int status = calcNEISpectrum(energyArray, Zinput, abundance, Redshift, Tinput, 
				IonFrac, true, velocity, flux, fluxErr);
  if ( status != 0 ) {
    std::stringstream oss;
    oss << "calcNEISpectrum failed in bvvrnei: status = " << status << "\n";
