      ...
      SUBROUTINE xsabsw(ear, ne, param, ifl, photar, photer)
...
c             kaa 9/28/01  increased accuracy for H ionization potential
...
c Now obsolete version of Wisconsin absorption - new version XSPHAB uses 
c individual element calculations from XSVPHB.

      INTEGER NRANGE
      PARAMETER (NRANGE=16)
      REAL LYLIM
      PARAMETER (LYLIM=0.0135984)
      INTEGER ir, ie
...
c if this is the first time through then write a warning message that this model
c is obsolete
     ...
     &        10)
         CALL xwrite(
     & "with historical results. The tbabs model should be used for",10)
         CALL xwrite(
     & "the ISM or phabs for general photoelectric absorption.",10)
         CALL xwrite(
     & "**************************************************************",
     &        10)
         first = .false.
